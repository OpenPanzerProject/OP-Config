...
SoundDevicesComboBox::SoundDevicesComboBox(QWidget *parent) : QComboBox(parent)
{
    ...
    // FOR FUTURE USE
    //insertItem(count(), "Onboard Sound (TCB MkII)", SD_ONBOARD);
    //insertItem(count(), "Beier USM-RC-2", SD_BEIER_USMRC2);
    // etc...

    // Connect to my custom signal
    connect(this, &SoundDevicesComboBox::currentIndexChanged, this, &SoundDevicesComboBox::CurrentDeviceChangedSlot);
}
